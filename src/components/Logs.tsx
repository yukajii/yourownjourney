...
const Logs = () => {
  ...
  const handleDelete = async (log: Log) => {
    const ok = await confirm({
      ...
      body:
        `${fmt(log.durationSec)} walked on ${when(log.timestamp)} will be removed ` +
        "and subtracted from your total.",
      confirmLabel: "Delete",
      danger: true,
    });
    if (ok) removeLog(log);
              ...
              <div className="flex items-baseline justify-between gap-3">
                ...
                <span className="flex shrink-0 items-baseline gap-2 font-mono tabular-nums">
                  ...
                  <button
                    ...
                    aria-label={`Edit entry from ${when(l.timestamp)}`}
                  ...
                  <button
                    ...
                    aria-label={`Delete entry from ${when(l.timestamp)}`}
